$(document).ready(function() {
  ...
  function readURL(input) {
    if (input.files && input.files[0]) {
      ...
      reader.onload =
          function(e) { $('#profile-display').attr('src', e.target.result); }

          reader.readAsDataURL(input.files[0]);
    }
  }

  $("#profile-upload").on("change", function() { readURL(this); });
  ...
  $('.cash-popup-btn')
      .on('click',
          function() { $('html').addClass('cash-method-popup-active'); });
  ...
  $('.card-popup-btn')
      .on('click',
          function() { $('html').addClass('card-method-popup-active'); });
  ...
  $('.toggle h4').on("click", function() {
    if ($(this).next().is(':hidden')) {
      ...
      $(this).toggleClass('active').next().slideDown(500).parent().toggleClass(
          "activate");
    }
  });
  ...
  if ($.isFunction($.fn.downCount)) {
    $('.countdown').downCount({date : '12/12/2018 12:00:00', offset : +5});
  }
...
jQuery(window).on('load', function($) {
  ...
  if (jQuery('.filter-item').length > 0) {
    ...
    jQuery(window).resize(function() {
      if (jQuery().isotope) {
        jQuery('.masonry')
            .isotope('layout'); // layout/relayout on window resize
      }
    });
  }
});
